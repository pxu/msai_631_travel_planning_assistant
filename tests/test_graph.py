...
from travel_assistant.graph import _extract_json, _is_confirmation, build_graph, format_summary
...
def test_single_message_extracts_all_required_fields_and_skips_to_summary(graph):
    ...
    message = "I want a 7-day family trip to Japan. We love food and theme parks. Our budget is moderate."
...
def test_explicit_dollar_amount_is_used_over_models_vague_paraphrase():
    ...
    assert "__interrupt__" in result
    payload = result["__interrupt__"][0].value
    assert payload["collected_fields"].get("budget_level") == "$4000"
...
def test_extraction_calls_use_deterministic_decoding(llm, graph):
    ...
    extraction_calls = [kwargs for prompt, kwargs in calls if "structured travel preferences" in prompt]
